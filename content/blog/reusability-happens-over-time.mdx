...
Instead of spending time and thought at the beginning of a project or architecture identifying the right "standard parts" to pull from the shelf later, we should focus on building the initial product, admitting that we don't know what we don't know. Admitting that bit is so important, as it paves the way toward a mindset of continual improvement and refactoring.
